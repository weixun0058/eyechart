...
### Requirement: 标准来源统一
系统 SHALL 以 `临床视力测试标准基线.md` 作为唯一标准来源。

#### Scenario: 标准冲突处理
- **WHEN** 需求规格、项目规划、设计稿与历史实现存在流程差异
- **THEN** 以 `临床视力测试标准基线.md` 中的 ETDRS 标准化流程为准
- **AND** 常见 5 米行读流程仅作为兼容映射，不作为默认测量引擎
...
#### Scenario: 标准化流程状态迁移测试
- **WHEN** 按 ETDRS 协议完成当前行逐视标作答
- **THEN** 系统正确统计该行正确数与错误数
- **AND** 系统按协议推进到下一行或结束测试
- **AND** ETDRS 分数与临床兼容行结果计算正确
