...
- [ ] Task A1: 固化 Android-only 边界与命令基线
  - [ ] SubTask A1.1: 在文档中声明 Android-only 验收边界
  - [ ] SubTask A1.2: 建立开发日常命令序列（核心测试）
  - [ ] SubTask A1.3: 建立提测前最小回归命令序列
  - 验证: 文档存在且命令可执行，输出被记录为证据

- [ ] Task A2: 收敛核心测试反馈回路
  - [ ] SubTask A2.1: 仅执行核心测试文件（staircase、math）
  - [ ] SubTask A2.2: 记录核心测试耗时并形成时间序列
  - [ ] SubTask A2.3: 若耗时异常，分析生成链或环境因素并出具调整建议
...
- [ ] Task A5: 代码生成触发策略调整
  - [ ] SubTask A5.1: 梳理模型/注解改动清单
  - [ ] SubTask A5.2: 调整本地流程，仅在上述改动时运行 build_runner
